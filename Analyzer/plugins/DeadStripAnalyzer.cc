...
class DeadStripAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit DeadStripAnalyzer(const edm::ParameterSet&);
...
DeadStripAnalyzer::DeadStripAnalyzer(const edm::ParameterSet& conf) : qualESToken(esConsumes()) {
...
void DeadStripAnalyzer::analyze(const edm::Event& event, const edm::EventSetup& es) {
...
void DeadStripAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;  
  descriptions.add("DeadStripAnalyzer", desc);
}

#include "FWCore/Framework/interface/MakerMacros.h"
DEFINE_FWK_MODULE(DeadStripAnalyzer);
